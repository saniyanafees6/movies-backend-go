...
func (m *DBModel) Get(id int) (*Movie, error) {
	...
	// get the genres
	query = `select mg.id, mg.genre_id, mg.movie_id, g.genre_name from movies_genres mg
	left join genres g
	on g.id = mg.genre_id
	where mg.movie_id = $1`
...
func (m *DBModel) All() ([]*Movie, error) {
	...
	for rows.Next(){
		...
		// get the genres
	genreQuery := `select mg.id, mg.genre_id, mg.movie_id, g.genre_name from movies_genres mg
	left join genres g
	on g.id = mg.genre_id
	where mg.movie_id = $1`
